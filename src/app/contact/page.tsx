    ...
    <div className="min-h-screen w-full bg-[#F7F7F7]">
      ...
      <section className="w-full flex flex-col md:flex-row h-[400px]">
        ...
        <div className="relative flex-1 flex items-center">
          ...
          <div className="relative z-20 flex flex-col items-start pl-30 pr-0 max-w-xl w-full">
            <div className="flex items-start mb-4">
              <div className="w-2 h-[60px] bg-[#3376C8] mr-4 rounded" />
            ...
            <button className="bg-[#3376C8] text-white font-bold px-8 py-3 rounded shadow hover:bg-blue-600 transition-all text-xs tracking-widest uppercase w-max" style={{fontFamily: 'Montserrat, Arial, sans-serif', letterSpacing: '0.08em'}}>
              Send a Message
            </button>
          </div>
        </div>
        {/* Right: Blue contact info box */}
        <div className="w-full md:w-[400px] bg-[#5B9DED] flex flex-col justify-center px-10 py-12">
          <h2 className="text-3xl font-extrabold mb-8 text-black" style={{ fontFamily: 'Montserrat, Arial, sans-serif' }}>Contact Info</h2>
          <div className="mb-6">
      ...
      <section className="w-full bg-white flex flex-col md:flex-row items-center justify-between px-16 py-10 border-b border-gray-200">
        <div className="flex flex-col items-start">
          <span className="text-[2.5rem] font-extrabold text-[#5B9DED] leading-tight mb-2" style={{ fontFamily: 'Montserrat, Arial, sans-serif' }}>
            Call: (541) 931-3526
          </span>
          <span className="text-[1.6rem] font-bold text-[#222]" style={{ fontFamily: 'Montserrat, Arial, sans-serif' }}>
            We Can't Wait to Make Your Ideas a Reality
          </span>
        </div>
        <div className="flex-1 flex justify-end w-full mt-8 md:mt-0">
          <button
            className="bg-white border-4 border-black px-10 py-3 font-bold text-sm tracking-widest shadow-[6px_8px_16px_0_rgba(0,0,0,0.18)] hover:bg-gray-100 transition-all uppercase text-[#5B9DED]"
      ...
      <section className="w-full flex justify-center items-start py-16 bg-[#F7F7F7]">
        <div className="flex flex-row w-full max-w-[1200px] gap-12 items-start">
          {/* Left: Form */}
          <div className="flex-1 bg-white px-10 py-10 flex flex-col justify-start min-w-[420px] max-w-[540px]" style={{ boxSizing: 'border-box' }}>
            <h2 className="text-[2.1rem] font-extrabold mb-2 text-[#222] leading-tight" style={{ fontFamily: 'Montserrat, Arial, sans-serif' }}>Get In Touch</h2>
            <div className="w-16 h-1 bg-blue-600 mb-8" />
            <form className="flex flex-col gap-5">
              <input type="text" placeholder="Name" className="bg-white border border-gray-200 rounded-none px-5 py-3 text-[1rem] font-medium text-gray-800 placeholder-gray-400 focus:ring-2 focus:ring-blue-400" style={{ fontFamily: 'Montserrat, Arial, sans-serif', height: 44 }} />
              <input type="email" placeholder="Email" className="bg-white border border-gray-200 rounded-none px-5 py-3 text-[1rem] font-medium text-gray-800 placeholder-gray-400 focus:ring-2 focus:ring-blue-400" style={{ fontFamily: 'Montserrat, Arial, sans-serif', height: 44 }} />
              <textarea placeholder="Message" rows={4} className="bg-white border border-gray-200 rounded-none px-5 py-3 text-[1rem] font-medium text-gray-800 placeholder-gray-400 focus:ring-2 focus:ring-blue-400 resize-none" style={{ fontFamily: 'Montserrat, Arial, sans-serif', minHeight: 90 }} />
              <div className="flex justify-start mt-2">
                <button type="submit" className="bg-white border-2 border-black px-8 py-2 font-bold text-xs tracking-widest shadow-[0_8px_16px_0_rgba(0,0,0,0.18)] hover:bg-gray-100 transition-all uppercase text-black" style={{ fontFamily: 'Montserrat, Arial, sans-serif', letterSpacing: '0.08em', boxShadow: '8px 12px 16px 0 rgba(0,0,0,0.18)' }}>
                  Send Message
                </button>
              </div>
            </form>
          ...
          {/* Right: FAQ/Accordion */}
          <div className="flex-1 bg-white px-10 py-10 min-w-[420px] max-w-[540px] flex flex-col justify-start" style={{ boxSizing: 'border-box' }}>
            {faqs.map((faq, idx) => (
              <div key={idx} className={"border-b border-gray-200 " + (idx === 0 ? "pt-0" : "pt-2") + (idx === faqs.length - 1 ? " border-b-0" : "")}>
                <button
                  className={`w-full text-left font-extrabold text-[1.15rem] py-5 text-[#222] focus:outline-none flex justify-between items-center`}
                ...
                {openIndex === idx && (
                  <div className="text-gray-500 text-[1rem] leading-relaxed pb-5 pl-1 pr-2" style={{ fontFamily: 'Montserrat, Arial, sans-serif', lineHeight: '1.6' }}>
                    {faq.answer}
                  </div>
                )}
              </div>
      ...
      <section className="w-full bg-[#F7F7F7]">
        <div className="w-full h-[260px] md:h-[340px] relative overflow-hidden">
          <iframe
